...
async def upload_files(
    ...
    Args:
        ...
        session_id: Session ID generated by frontend for tracking
    ...
    uploaded_files = []
    errors = []
    s3_keys = []  # Track S3 keys for Textract
    ...
    for file in files:
        try:
            ...
            s3_keys.append(result.get('s3_key'))  # Collect S3 keys
...
def _store_project_metadata(
    ...
    Args:
        ...
        s3_keys: List of S3 keys for uploaded files
    """
    import boto3
    ...
    # Store S3 keys as a list
    if s3_keys:
        item['s3_keys'] = {'L': [{'S': key} for key in s3_keys if key]}
    
    # Also create an upload_id mapping for quick lookup
    upload_mapping_item = {
        'PK': {'S': f'UPLOAD#{upload_id}'},
        'SK': {'S': 'MAPPING'},
        'entity_type': {'S': 'upload_mapping'},
        'upload_id': {'S': upload_id},
        'project_id': {'S': project_id},
        's3_keys': {'L': [{'S': key} for key in s3_keys if key]},
        'created_at': {'S': timestamp}
    }
    
    if session_id:
        upload_mapping_item['session_id'] = {'S': session_id}
